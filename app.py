...
# ------------------------------
st.set_page_config(
    page_title="SPK Laptop SAW", layout="wide", initial_sidebar_state="expanded"
)
...
st.markdown(
    "<p class='sub-header'>Metode Simple Additive Weighting – Dataset laptop_prices.csv</p>",
    unsafe_allow_html=True,
)
...
def preprocess_data(df):
    ...
    df_clean["Nama Laptop"] = (
        df_clean["brand_name"].astype(str) + " " + df_clean["model"].astype(str)
    )
    df_clean = df_clean.drop_duplicates(subset=["Nama Laptop"])
    df_clean = df_clean.dropna(
        subset=["price", "ram_num", "memory_size", "cpu_str", "gpu_str"]
    )
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "Unggah dataset (CSV / Excel)", type=["csv", "xlsx"]
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            if file_ext == "csv":
                ...
                if df_raw is None:
                    st.error(
                        "Tidak dapat membaca file CSV dengan encoding standar. Coba konversi file ke UTF-8."
                    )
                    st.stop()
            else:
                ...
                if len(df_raw.columns) == 1:
                    ...
                    if "," in sample or ";" in sample or "\t" in sample:
                        ...
                        try:
                            df_raw = pd.read_csv(
                                io.StringIO(csv_str), sep=None, engine="python"
                            )
                        except:
                            df_raw = pd.read_csv(
                                io.StringIO(csv_str), delimiter=";", engine="python"
                            )
            ...
            st.success(
                f"Dataset berhasil dimuat! {df_raw.shape[0]} baris, {df_raw.shape[1]} kolom"
            )
...
with st.sidebar:
    ...
    if min_price == max_price:
        ...
        default_max = min(max_price, 3499)
    ...
    st.caption(
        f"Rentang: {format_price_usd(price_range[0])} – {format_price_usd(price_range[1])}"
    )

    # 2. Filter Merek (Manual)
    brands = sorted(df_clean["brand_name"].unique())
    selected_brands = st.multiselect(
        "Pilih Merek (kosongkan = gunakan filter 'Jumlah Merek Teratas')",
        brands,
        default=[],
    )

    # 4. Terapkan Filter ke DataFrame
    ...
    if len(df_filtered) == 0:
        st.warning(
            "Tidak ada laptop yang cocok dengan filter. Ubah filter atau lihat data dummy."
        )
        df_filtered = df_clean.head(1)  # dummy

    # 5. Slider Jumlah Laptop yang Diproses
    total_rows = len(df_filtered)
    min_limit = 1
    max_limit = max(1, total_rows)
    default_value = max_limit

    rows_limit = st.slider(
        "Jumlah Laptop yang Diproses",
        min_value=min_limit,
        max_value=max_limit,
        value=default_value,
        step=1,
    ...
    st.caption(
        f"Menampilkan **{len(df_filtered)}** laptop dari **{total_rows}** total "
        f"(setelah filter harga dan merek)."
    )
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Eksplorasi Data", "Kriteria dan Bobot", "Perhitungan SAW", "Peringkat"]
)
...
with tab1:
    ...
    with col1:
        ...
        st.metric(
            "Rentang Harga",
            f"{format_price_usd(df_filtered['price'].min())} – {format_price_usd(df_filtered['price'].max())}",
        )
    ...
    fig_scatter = px.scatter(
        ...
        y="price",
        color="brand_name",
        hover_data=["Nama Laptop"],
        title="RAM vs Harga",
        labels={"ram_num": "RAM (GB)", "price": "Harga (USD)"},
    )
...
with tab2:
    ...
    available = {
        k: v
        for k, v in candidate_criteria.items()
    ...
    if not st.session_state.selected_criteria:
        ...
        st.session_state.selected_criteria = [
            k for k in default_selected if k in available
        ]
    ...
    if "apply_profile" in st.session_state and st.session_state.apply_profile:
        ...
        profile_options = {
            ...
            "Pekerja Kantoran (Fokus Keseimbangan)": {
                "price": 0.35,
                "ram_num": 0.20,
                "memory_size": 0.20,
                "cpu_score": 0.20,
        ...
        if prof_name in valid_profiles:
            ...
            if total > 0:
                ...
                for crit in selected:
                    st.session_state[f"weight_{crit}"] = float(
                        prof_weights.get(crit, 0.0)
                    )
    ...
    for i, crit in enumerate(selected):
        with cols[i]:
            ...
            is_benefit = st.checkbox(
                "Benefit",
                value=available[crit]["default_benefit"],
                key=f"benefit_{crit}",
            )
    ...
    weight_df = pd.DataFrame(
        [
            {
                "Kriteria": available[crit]["label"],
                "Bobot": weights[crit]["weight"],
                "Tipe": "Benefit" if weights[crit]["benefit"] else "Cost",
            }
            for crit in selected
        ]
    )
...
with tab3:
    ...
    fig = px.bar(
        norm_df.sort_values("Skor SAW", ascending=False).head(20),
        x="Nama Laptop",
        y="Skor SAW",
        title="20 Laptop dengan Skor Tertinggi",
...
with tab4:
    ...
    st.success(
        f"Laptop Terbaik untuk profil yang dipilih: {ranking.iloc[0]['Nama Laptop']} (Skor: {ranking.iloc[0]['Skor SAW']:.4f})"
    )
    ...
    for col in display_ranking.columns:
        if col == "price":
            column_config[col] = st.column_config.TextColumn(
                "Harga (USD)", help="Harga dalam Dolar AS"
            )
        elif col == "Nama Laptop":
            column_config[col] = st.column_config.TextColumn(
                "Nama Laptop", width="large"
            )
        elif col == "Skor SAW":
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        excel_ranking.to_excel(
            writer, sheet_name="3. Hasil Peringkat Akhir", index=True
        )
    ...
    with st.expander("Perbandingan Detail Top 5"):
        ...
        st.dataframe(
            display_top5, column_config=column_config, use_container_width=True
        )
